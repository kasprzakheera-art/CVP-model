...
if input_mode == "⌨️ 手动精细录入 (单场沙盘)":
    ...
    if uploaded_file is not None:
        try:
            ...
            if host_commission > 1:
                host_commission = host_commission / 100.0
...
if data_ready:
    ...
    with tab1:
        ...
        if unit_margin > 0:
            ...
            idx = min(range(len(x_values)), key=lambda i: abs(x_values[i] - Q))
            fig.add_trace(go.Scatter(x=[Q], y=[revenue_values[idx]], mode='markers', name='目标发货点', marker=dict(color='green', size=12, symbol='star')))
